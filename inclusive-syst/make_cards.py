...
def get_envelope(samp, passed, ptbin, obs, syst):
    # open root file of histograms                                                                       
    f = ROOT.TFile.Open("signalregion.root")
    ...
    for i in range(1,hup.GetNbinsX()+1):
        ...
        do = hdo.GetBinContent(i)

        if abs(up-1) > abs(do-1):
            sumw += []
            sumw2 += [hup.GetBinError(i)*hup.GetBinError(i)]
        else:
            sumw += [abs(do-1)]
            sumw2 += [hdo.GetBinError(i)*hup.GetBinError(i)]
...
def test_rhalphabet(tmpdir):
    ...
    btagWeight = rl.NuisanceParameter('CMS_btagWeight', 'shape')
    btagEffStat = rl.NuisanceParameter('CMS_btagEffStat', 'shape')
    ...
    for ptbin in range(npt):
        for region in ['pass', 'fail']:
            ...
            for sName in samps:
                ...
                jet_trigger_up = get_template(sName, isPass, ptbin+1, obs=msd, syst="jet_triggerUp")[0]       
                jet_trigger_down = get_template(sName, isPass, ptbin+1, obs=msd, syst="jet_triggerDown")[0]   
                sample.setParamEffect(jet_trigger, jet_trigger_up, jet_trigger_down)  

                jes_up = get_template(sName, isPass, ptbin+1, obs=msd, syst="JESUp")[0]
                jes_down = get_template(sName, isPass, ptbin+1, obs=msd, syst="JESDown")[0]
                sample.setParamEffect(jes, jes_up, jes_down)
                
                jer_up = get_template(sName, isPass, ptbin+1, obs=msd, syst="JERUp")[0]
                jer_down = get_template(sName, isPass, ptbin+1, obs=msd, syst="JERDown")[0]
                sample.setParamEffect(jer, jer_up, jer_down)

                ues_up = get_template(sName, isPass, ptbin+1, obs=msd, syst="UESUp")[0]
                ues_down = get_template(sName, isPass, ptbin+1, obs=msd, syst="UESDown")[0]
                sample.setParamEffect(ues, ues_up, ues_down)

                btagWeight_up = get_template(sName, isPass, ptbin+1, obs=msd, syst="btagWeightUp")[0]
                btagWeight_down = get_template(sName, isPass, ptbin+1, obs=msd, syst="btagWeightDown")[0]
                sample.setParamEffect(btagWeight, btagWeight_up, btagWeight_down)
                
                btagEffStat_up =get_template(sName, isPass, ptbin+1, obs=msd, syst="btagEffStatUp")[0]
                btagEffStat_down = get_template(sName, isPass, ptbin+1, obs=msd, syst="btagEffStatDown")[0]
