...
def passfailSF(isPass, sName, ptbin, cat, obs, mask, SF=1, SF_unc=0.1, muon=False):
    ...
    if isPass:
        ...
        _pass = get_template(sName, 1, ptbin, cat+'_', obs=obs, syst='nominal', muon=muon)
        _pass_rate = np.sum(_pass[0] * mask)

        _fail = get_template(sName, 0, ptbin, cat+'_', obs=obs, syst='nominal', muon=muon)
...
#                        sf,sfunc = passfailSF(isPass, sName, -1, cat, msd, mask, 1, 0.3)
#                        sample.scale(sf)
...
#                        sample.setParamEffect(sys_scale,mtempl.get(shift=7.)[0],mtempl.get(shift=-7.)[0],scale=realshift / 7.)
    ...
    for ptbin in range(npt):
        for cat in cats:
            ...
            failCh.addSample(fail_qcd)
            pass_qcd = rl.TransferFactorSample('ptbin%d%spass_qcd' % (ptbin, cat), rl.Sample.BACKGROUND, tf_params[ptbin, :], fail_qcd)
            passCh.addSample(pass_qcd)
