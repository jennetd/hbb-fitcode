...
void draw(int index, bool pass, bool log=true){
  ...
  data_obs = (TH1D*)dataf->Get(("vbf_fail_pt"+to_string(index+1)+"_data_nominal").c_str());
  if( pass )
    data_obs = (TH1D*)dataf->Get(("vbf_pass_pt"+to_string(index+1)+"_data_nominal").c_str());
  ...
  string name = "ptbin"+to_string(index)+"vbffail";
  if( pass )
    name = "ptbin"+to_string(index)+"vbfpass";
  ...
  cout << "Zjets:"    << Zjetsbb->Integral() << endl;
  ...
  string text = "DDB fail, VBF cat.";
  if( pass )
    text = "DDB pass, VBF cat.";
