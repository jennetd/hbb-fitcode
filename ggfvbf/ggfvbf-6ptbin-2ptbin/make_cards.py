...
def passfailSF(isPass, sName, ptbin, cat, obs, mask, SF=1, SF_unc=0.1, muon=False):
    ...
    if isPass:
        ...
        _pass = get_template(sName, 1, ptbin+1, cat+'_', obs=obs, syst='nominal', muon=muon)
        _pass_rate = np.sum(_pass[0] * mask)

        _fail = get_template(sName, 0, ptbin+1, cat+'_', obs=obs, syst='nominal', muon=muon)
...
#    sys_smear = rl.NuisanceParameter('CMS_hbb_smear_{}'.format(year), 'shape')
    ...
    ptbins['vbf'] = np.array([450, 550, 1200])
    npt = {}
    ...
    for cat in cats:
        ...
        if cat == 'ggf':
            ...
            print(initial_vals)
            tf_MCtempl = rl.BernsteinPoly('tf_MCtempl_'+cat, (2, 2), ['pt', 'rho'], init_params=initial_vals, limits=(-20, 20))
        else:
        ...
        if cat == 'ggf':
            tf_dataResidual = rl.BernsteinPoly('tf_dataResidual_'+cat, (2, 2), ['pt', 'rho'], limits=(-20, 20))
        else:
        ...
        for ptbin in range(npt[cat]):
            for region in ['pass', 'fail']:
                ...
                for sName in samps:
                    ...
                    templates[sName] = get_template(sName, isPass, ptbin+1, cat+'_', obs=msd, syst='nominal') 
                ...
                data_obs = get_template('data', isPass, ptbin+1, cat+'_', obs=msd, syst='nominal')
