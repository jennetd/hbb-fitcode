...
void draw(int index, bool pass, bool log=true){
  ...
  data_obs = (TH1D*)dataf->Get(("fail_mjj"+to_string(index+1)+"_data_nominal").c_str());
  if( pass )
    data_obs = (TH1D*)dataf->Get(("pass_mjj"+to_string(index+1)+"_data_nominal").c_str());
  ...
  string name = "ptbin0mjjbin"+to_string(index)+"fail";
  if( pass )
    name = "ptbin0mjjbin"+to_string(index)+"pass";
  ...
  bkgHiggs->Reset();
  bkgHiggs->Add((TH1D*)f->Get((histdirname+"/ggF").c_str()));
  ...
  /* VV */
  TH1D* VV = (TH1D*)bkgHiggs->Clone("VV");
  VV->Reset();
  ...
  /* single t */
  TH1D* singlet = (TH1D*)bkgHiggs->Clone("singlet");
  singlet->Reset();
  ...
  string text = "DDB failing, m_{jj} bin "+to_string(index+1);
  if( pass )
    text = "DDB passing, m_{jj} bin "+to_string(index+1);
